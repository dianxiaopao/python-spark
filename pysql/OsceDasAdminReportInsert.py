...
if __name__ == '__main__':
    ...
    sc = SparkContext(appName="OsceDasAdminReportInsert")
    ...
    try:
        ...
        for row in ds_slave.collect():
            src_fields = {'osce_das_admin_report': ['adminid', 'examid', 'report', 'creattime']}
            src_fields = json.dumps(src_fields)
            src_fieldsvul ="osce_das_admin_report.%s|osce_das_admin_report.%s|osce_das_admin_report.%s|osce_das_admin_report.%s" \
                           % (row.adminid, row.examid, row.report, row.creattime)
            ...
            sql = "insert into  %s  select A.* from (select '%s' as adminid," \
                  "'%s' as examid ," \
                  "'%s' as report ," \
                  "'%s' as cust_no," \
                  "'%s' as isvalid," \
                  ...
                  "'%s' as createts," \
                  "'%s' as updatets ) A"\
                  % (etsTempTable, row.adminid, row.examid, row.report,
