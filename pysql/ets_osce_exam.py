...
def md5(row):
    ...
    reload(sys)
    sys.setdefaultencoding('utf-8')
    temstr = "osce_exam.%s|osce_exam.%s|osce_exam.%s|osce_exam.%s|osce_exam.%s|osce_exam.%s" \
             % (row.id, row.name, str(row.begintime), str(row.endtime), str(row.examtype), str(row.updatetime))
    m = hashlib.md5()
    m.update(temstr)
    return m.hexdigest()
...
def do_ets_task(sc, ets_dburl_env, wfc):
    ...
    try:
        if max_updates is not None:
            ...
            slave_sql = " select id, name, begintime, endtime, updatetime, examtype " \
                        "  from  %s  where `updatetime` > '%s' " % (slaveTempTable, max_updates)
        else:
            print(u"本次为初次抽取")
            slave_sql = " select id, name, begintime, endtime, examtype, updatetime " \
        ...
        print(u'开始组装数据...')
        # src_fields = json.dumps({'osce_exam': ['id', 'name', 'begintime', 'endtime', 'examtype', 'updatetime']})
        # 字段值
        filedvlue = ds_slave.map(lambda row: (row.id, row.name, str(row.begintime), str(row.endtime), str(row.examtype),
                                              cust_no, isvalid,
                                              md5(row), now_time, str(row.updatetime)))
        # 创建列
        schemaString = "id,name,begintime,endtime,examtype,cust_no,isvalid,src_fields_md5,createts,updatets"
