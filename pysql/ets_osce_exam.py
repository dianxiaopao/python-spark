...
def do_ets_task(sc, ets_dburl_env, wfc):
    ...
    ets_url = ets_dburl_env_dict.get('ets_osce_exam', '').get('dst', '')
    slave_url = ets_dburl_env_dict.get('ets_osce_exam', '').get('src', '')
    ...
    try:
        ...
        # 创建列
        schemaString = "id,name,begintime,endtime,cust_no,isvalid,src_fields_md5,createts,updatets"
