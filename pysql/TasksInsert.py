...
if __name__ == '__main__':
    ...
    cust_no = '1'
    isvalid = '1'
    sc = SparkContext(appName="TasksInsert")
    ...
    dff.registerTempTable('tasks_slave')
    ...
    dft = sqlContext.read.format("jdbc").options(url="jdbc:mysql://192.168.1.200:3307/bd_ets?user=root"
                                                     ...
                                                     "&zeroDateTimeBehavior=convertToNull", dbtable="ets_tasks",
                                                       driver="com.mysql.jdbc.Driver").load()
    dft.registerTempTable('ets_tasks')
    ds_ets = sqlContext.sql(" select max(updatets) as max from ets_tasks ")
    pp = ds_ets.collect()[0]
    max_updates = pp.max
    slave_sql = ''
    try:
        if max_updates is not None:
            ...
            slave_sql = " select id, schedule_id, updated_at from tasks_slave where `updated_at` > '%s' " % (max_updates)
        else:
            logging.info(u"本次为初次抽取")
            slave_sql = " select id, schedule_id, updated_at from tasks_slave "
        ...
        for row in ds_slave.collect():
            ...
            sql = "insert into  ets_tasks  select A.* from (select '%s' as id," \
                  "'%s' as schedule_id ," \
                  "'%s' as cust_no," \
                  "'%s' as isvalid," \
                  "'%s' as src_fields," \
                  ...
                  "'%s' as createts," \
                  "'%s' as updatets ) A"\
                  % (row.id, row.schedule_id, cust_no, isvalid,  src_fields, src_fields_md5,
