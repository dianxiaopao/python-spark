...
def do_ets_task(sc, ets_dburl_env, wfc):
    ...
    cust_no = '1'
    isvalid = '1'
    slaveTempTable = 'osce_das_student_report'
    etsTempTable = wfc
    ets_dburl_env_dict = loadjson(ets_dburl_env)
    ets_url = ets_dburl_env_dict.get('ets_osce_das_student_report', '').get('dst', '')
    slave_url = ets_dburl_env_dict.get('ets_osce_das_student_report', '').get('src', '')
    print "**************************"
