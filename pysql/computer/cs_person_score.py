...
def do_cs_task(sc, cs_dburl_env):
    ...
    try:
        dbtable = load_source('get_which_for_cs', os.path.join(os.path.dirname(__file__), 'Utils.py')).get_which_for_cs('task_ets_score', config.get('ets_score', ''))
        ets_score = sqlContext.read.format("jdbc").options(url=config.get('ets_score', ''), dbtable=dbtable,
                                                           driver=driver).load()
        ets_score.registerTempTable('ets_score')

        dbtable = load_source('get_which_for_cs', os.path.join(os.path.dirname(__file__), 'Utils.py')).get_which_for_cs(
            'task_ets_learn', config.get('ets_learn', ''))
        ets_learn = sqlContext.read.format("jdbc").options(url=config.get('ets_learn', ''), dbtable=dbtable,
                                                           driver=driver).load()
        ets_learn.registerTempTable('ets_learn')
        dbtable = load_source('get_which_for_cs', os.path.join(os.path.dirname(__file__), 'Utils.py')).get_which_for_cs(
            'task_ets_osce_das_admin_report', config.get('ets_score', ''))
        ets_osce_das_admin_report = sqlContext.read.format("jdbc").options(url=config.get('ets_osce_das_admin_report', ''), dbtable=dbtable,
                                                                           driver=driver).load()
        ...
        ets_apply_student = sqlContext.read.format("jdbc").options(url=config.get('ets_apply_student', ''), dbtable=dbtable,
                                                                   driver=driver).load()
